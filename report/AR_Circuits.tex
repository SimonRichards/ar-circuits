...
One challenge with all visualisations is dealing with different data ranges. Currently, no automatic scaling of voltage and current values is implemented. This means that the user must choose component values which result in values in a certain range, or the voltage-current visualisation might be so far from the markers as to be out of view, or the variance so small as to appear uniform. A related problem is when 
